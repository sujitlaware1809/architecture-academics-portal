...
export default function ArchitectDashboardLayout({ children }: DashboardLayoutProps) {
  ...
  const sidebarItems = [
    { name: "Dashboard", href: "/architect-dashboard", icon: LayoutDashboard },
    { name: "Projects", href: "/architect-dashboard/projects", icon: Building2 },
    { name: "Certifications", href: "/architect-dashboard/certifications", icon: Award },
    { name: "Resources", href: "/architect-dashboard/resources", icon: FolderOpen },
          ...
          >
            <div className="p-4 border-b border-gray-200">
              <div className="flex items-center">
                ...
                <h1 className="ml-3 text-xl font-semibold text-gray-800">Architect Panel</h1>
              </div>
            </div>
              ...
              {sidebarItems.map((item) => {
                ...
                return (
                  <Link
                    key={item.name}
                    href={item.href}
                    className="flex items-center px-3 py-2 text-sm font-medium rounded-md hover:bg-green-50 hover:text-green-700 text-gray-700"
          ...
          {sidebarItems.map((item) => {
            ...
            return (
              <Link
                key={item.name}
                href={item.href}
                className={`group relative flex items-center ${sidebarOpen ? 'px-3' : 'justify-center'} py-2.5 text-sm font-medium rounded-lg hover:bg-green-50 hover:text-green-700 text-gray-600 transition-all duration-200`}
              >
