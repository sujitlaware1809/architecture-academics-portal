...
export default function RecruiterDashboardLayout({ children }: DashboardLayoutProps) {
  ...
  const sidebarItems = [
    ...
    { name: "Post a Job", href: "/recruiter-dashboard/post-job", icon: PlusCircle },
    { name: "My Listings", href: "/recruiter-dashboard/jobs", icon: Briefcase },
    { name: "Applications", href: "/recruiter-dashboard/applications", icon: Users },
          ...
          >
            <div className="p-4 border-b border-gray-200">
              <div className="flex items-center">
                ...
                <h1 className="ml-3 text-xl font-semibold text-gray-800">Recruiter Panel</h1>
              </div>
            </div>
          ...
          {sidebarItems.map((item) => {
            ...
            return (
              <Link
                key={item.name}
                href={item.href}
                className={`group relative flex items-center ${sidebarOpen ? 'px-3' : 'justify-center'} py-2.5 text-sm font-medium rounded-lg hover:bg-blue-50 hover:text-blue-700 text-gray-600 transition-all duration-200`}
              >
